...
class Kit_Credential_Hooks {
	...
	public function maybe_update_credentials( $result, $client_id ): void {
		...
		Audit_Log::add( 'credentials_updated_from_api_event', array( 'source' => 'oauth_or_refresh' ) );
	}
	...
	public function maybe_delete_credentials( $error, $client_id ): void {
		...
		if ( $settings->using_convertkit_credentials() ) {
			Audit_Log::add( 'invalid_token_ignored_convertkit_owned' );
			return;
		}
		...
		if ( 401 !== $code ) {
			Audit_Log::add( 'invalid_token_ignored_non_401', array( 'code' => (string) $code ) );
			return;
		}
		...
		Audit_Log::add( 'invalid_token_detected', array( 'count' => (string) $state['count'] ), 'warning' );
		...
		Audit_Log::add( 'invalid_token_threshold_reached_credentials_deleted', array(), 'error' );
	}
	...
	public function refresh_kit_access_token(): void {
		...
		if ( ! $settings->has_access_and_refresh_token() ) {
			Audit_Log::add( 'refresh_skipped_no_tokens' );
			return;
		}

		if ( $settings->using_convertkit_credentials() ) {
			wp_clear_scheduled_hook( Kit_Settings::CRON_REFRESH_HOOK );
			Audit_Log::add( 'refresh_skipped_convertkit_owned' );
			return;
		}

		if ( get_transient( self::REFRESH_LOCK_KEY ) ) {
			Audit_Log::add( 'refresh_skipped_lock_present' );
			return;
		}
		...
		try {
			...
			if ( is_wp_error( $result ) ) {
				Audit_Log::add( 'refresh_failed', array( 'error' => $result->get_error_message() ), 'warning' );
				return;
			}

			$settings->update_credentials( $result );
			Audit_Log::add( 'refresh_succeeded' );
		} finally {
